        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                <h3 class="text-lg font-semibold text-gray-900 mb-6">
                    ...
                    الصور الحالية ({{ $product->images->count() }})
                </h3>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select id="main_category" name="main_category_id" required
                            ...
                            @foreach($categories->whereNull('parent_id') as $category)
                                <option value="{{ $category->id }}" {{ old('category_id', $product->category->parent_id ?? $product->category_id) == $category->id ? 'selected' : '' }}>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div id="subcategory_section" style="display: {{ $product->category->parent_id ? 'block' : 'none' }}">
            ...
            <div id="shoe_sizes_section" class="bg-white p-6 rounded-xl shadow-sm border border-gray-200" style="display: none;">
                ...
                <div id="shoe_sizes_container">
                    @if($product->sizes && $product->sizes->count() > 0)
                        ...
                        <div class="shoe-size-row grid grid-cols-3 gap-3 mb-3" data-index="{{ $index }}">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">المقاس</label>
                                <select name="shoe_sizes[{{ $index }}][size]" class="shoe-size-select w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-orange-500 focus:border-orange-500" required>
                                    <option value="">اختر المقاس</option>
                                    <option value="35" {{ $size->size == '35' ? 'selected' : '' }}>35</option>
                                    <option value="36" {{ $size->size == '36' ? 'selected' : '' }}>36</option>
                                    <option value="37" {{ $size->size == '37' ? 'selected' : '' }}>37</option>
                                    <option value="38" {{ $size->size == '38' ? 'selected' : '' }}>38</option>
                                    <option value="39" {{ $size->size == '39' ? 'selected' : '' }}>39</option>
                                    <option value="40" {{ $size->size == '40' ? 'selected' : '' }}>40</option>
                                    <option value="41" {{ $size->size == '41' ? 'selected' : '' }}>41</option>
                                    <option value="42" {{ $size->size == '42' ? 'selected' : '' }}>42</option>
                                    <option value="43" {{ $size->size == '43' ? 'selected' : '' }}>43</option>
                                    <option value="44" {{ $size->size == '44' ? 'selected' : '' }}>44</option>
                                    <option value="45" {{ $size->size == '45' ? 'selected' : '' }}>45</option>
                                    <option value="46" {{ $size->size == '46' ? 'selected' : '' }}>46</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">الكمية</label>
                                <input type="number" name="shoe_sizes[{{ $index }}][quantity]" value="{{ $size->stock_quantity }}" min="0" class="shoe-quantity w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-orange-500 focus:border-orange-500" required>
                            </div>
...
@push('scripts')
<script>
    let shoeSizeIndex = {{ $product->sizes ? $product->sizes->count() : 1 }};
