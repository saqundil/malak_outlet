...
@section('title', 'إدارة المدن الأردنية')
@section('page-title', 'إدارة المدن الأردنية')
@section('page-description', 'إدارة المدن وتكاليف التوصيل في المملكة الأردنية الهاشمية')
...
<div class="space-y-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="stats-card" style="--bg-color-1: #3b82f6; --bg-color-2: #1e40af;">
            ...
            <h3 class="text-2xl font-bold mb-2">{{ number_format($stats['total_cities']) }}</h3>
        ...
        <div class="stats-card" style="--bg-color-1: #10b981; --bg-color-2: #059669;">
            ...
            <h3 class="text-2xl font-bold mb-2">{{ number_format($stats['active_cities']) }}</h3>
        ...
        <div class="stats-card" style="--bg-color-1: #f59e0b; --bg-color-2: #d97706;">
            ...
            <h3 class="text-2xl font-bold mb-2">{{ number_format($stats['total_orders']) }}</h3>
        ...
        <div class="stats-card" style="--bg-color-1: #8b5cf6; --bg-color-2: #7c3aed;">
            ...
            <h3 class="text-2xl font-bold mb-2">{{ number_format($stats['total_delivery_revenue'], 2) }}</h3>
    ...
    <div class="bg-white p-6 rounded-xl shadow-lg">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.cities.create') }}" class="action-btn btn-success">
        ...
        <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            ...
            <div>
                <select name="sort_by" class="search-box w-full px-4 py-3">
                    ...
                    <option value="delivery_cost" {{ request('sort_by') === 'delivery_cost' ? 'selected' : '' }}>تكلفة التوصيل</option>
                    <option value="delivery_days" {{ request('sort_by') === 'delivery_days' ? 'selected' : '' }}>أيام التوصيل</option>
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('admin.cities.index') }}" class="action-btn btn-warning">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <input type="checkbox" class="city-checkbox rounded" value="{{ $city->id }}">
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center">
                                ...
                                <div>
                                    <div class="text-sm font-medium text-gray-900">{{ $city->display_name }}</div>
                                    @if($city->name_en)
                                        <div class="text-sm text-gray-500">{{ $city->name_en }}</div>
                                    @endif
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-bold text-green-600">{{ $city->formatted_delivery_cost }}</div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">
                                {{ number_format($city->orders_count) }}
                                @if($city->orders_count > 0)
                                    <a href="{{ route('admin.cities.orders', $city) }}" class="text-blue-500 hover:text-blue-700 mr-2">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('admin.cities.show', $city) }}" class="action-btn btn-primary">
                                ...
                                <button onclick="toggleStatus({{ $city->id }})" 
                                        class="action-btn {{ $city->is_active ? 'btn-danger' : 'btn-success' }}">
                                    <i class="fas fa-{{ $city->is_active ? 'ban' : 'check' }}"></i>
                                </button>
                                @if($city->orders_count == 0)
                                <button onclick="deleteCity({{ $city->id }})" class="action-btn btn-danger">
