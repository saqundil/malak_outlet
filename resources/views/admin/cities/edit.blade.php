...
@section('title', 'تعديل المدينة - ' . $city->display_name)
@section('page-title', 'تعديل المدينة')
@section('page-description', 'تعديل معلومات وإعدادات مدينة ' . $city->display_name)
...
<div class="space-y-8">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-600 mt-2">تحديث معلومات وإعدادات {{ $city->display_name }}</p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
            <div class="form-card">
                ...
                <form action="{{ route('admin.cities.update', $city) }}" method="POST" id="cityForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="form-group">
                            ...
                            <input 
                                ...
                                id="name_en" 
                                name="name_en" 
                                class="form-input {{ $errors->has('name_en') ? 'error' : '' }}"
                        ...
                        <div class="form-group">
                            ...
                            <input 
                                ...
                                id="delivery_cost" 
                                name="delivery_cost" 
                                class="form-input {{ $errors->has('delivery_cost') ? 'error' : '' }}"
                                value="{{ old('delivery_cost', $city->delivery_cost) }}"
                                placeholder="0.00"
                                step="0.01"
                                min="0"
                                required
                        ...
                        <div class="form-group">
                            ...
                            <select id="delivery_days" name="delivery_days" class="form-input {{ $errors->has('delivery_days') ? 'error' : '' }}">
                                <option value="1" {{ old('delivery_days', $city->delivery_days) == 1 ? 'selected' : '' }}>يوم واحد</option>
                                <option value="2" {{ old('delivery_days', $city->delivery_days) == 2 ? 'selected' : '' }}>يومين</option>
                                <option value="3" {{ old('delivery_days', $city->delivery_days) == 3 ? 'selected' : '' }}>3 أيام</option>
                                <option value="4" {{ old('delivery_days', $city->delivery_days) == 4 ? 'selected' : '' }}>4 أيام</option>
                                <option value="5" {{ old('delivery_days', $city->delivery_days) == 5 ? 'selected' : '' }}>5 أيام</option>
                                <option value="7" {{ old('delivery_days', $city->delivery_days) == 7 ? 'selected' : '' }}>أسبوع</option>
                                <option value="10" {{ old('delivery_days', $city->delivery_days) == 10 ? 'selected' : '' }}>10 أيام</option>
                                <option value="14" {{ old('delivery_days', $city->delivery_days) == 14 ? 'selected' : '' }}>أسبوعين</option>
                            </select>
                            @error('delivery_days')
                                <div class="form-error">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="form-group">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <span class="text-gray-700" id="statusText">
                                {{ old('is_active', $city->is_active) ? 'نشط' : 'غير نشط' }}
                            </span>
                        </div>
                ...
                <form action="{{ route('admin.cities.destroy', $city) }}" method="POST" id="deleteForm" class="hidden">
                    @csrf
                    @method('DELETE')
                </form>
            </div>
                ...
                <div id="cityPreview" class="preview-card">
                    <div class="flex items-center justify-between mb-3">
                        <div class="flex items-center">
                            ...
                            <div>
                                <div class="font-bold text-gray-900" id="previewName">{{ $city->display_name }}</div>
                                <div class="text-sm text-gray-500" id="previewNameEn">{{ $city->name_en ?: 'لا يوجد اسم إنجليزي' }}</div>
                            </div>
                        </div>
                        <div id="previewStatus" class="px-2 py-1 text-xs rounded-full {{ $city->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                    ...
                    <div class="space-y-2 text-sm">
                        <div class="flex justify-between">
                            ...
                            <span class="font-bold text-green-600" id="previewCost">{{ number_format($city->delivery_cost, 2) }} د.أ</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-600">مدة التوصيل:</span>
                            <span class="font-medium text-orange-600" id="previewDays">{{ $city->delivery_days }} {{ $city->delivery_days == 1 ? 'يوم' : 'أيام' }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="form-card">
                ...
                <div class="space-y-4">
                    <div class="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
                        ...
                        <span class="text-blue-900 font-bold">{{ $city->orders_count ?? 0 }}</span>
                    </div>
                    
                    <div class="flex items-center justify-between p-3 bg-green-50 rounded-lg">
                        <span class="text-green-700 font-medium">إيرادات التوصيل</span>
                        <span class="text-green-900 font-bold">{{ number_format(($city->orders_count ?? 0) * $city->delivery_cost, 2) }} د.أ</span>
                    </div>
                    
                    <div class="flex items-center justify-between p-3 bg-orange-50 rounded-lg">
                        <span class="text-orange-700 font-medium">آخر طلب</span>
                        <span class="text-orange-900 font-medium">
                            @if($city->orders()->latest()->first())
                                {{ $city->orders()->latest()->first()->created_at->diffForHumans() }}
                            @else
                                لا يوجد
                            @endif
                        </span>
                ...
                <div class="stats-mini mt-4">
                    ...
                    <div class="text-lg font-bold text-amber-900">
                        {{ number_format($city->orders_count / max(1, $city->created_at->diffInMonths(now()) ?: 1), 1) }}
                    </div>
                </div>
                @endif
            </div>
