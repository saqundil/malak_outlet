        ...
        <div class="container mx-auto px-4 py-4 sm:py-6">
            ...
            <div class="block sm:hidden">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <a href="{{ route('products.index') }}" class="bg-orange-500 text-white px-3 py-2 rounded-lg hover:bg-orange-600 transition-colors font-medium text-sm">
                        تسوق الآن
                    </a>
                </div>
            ...
            <div class="hidden sm:flex items-center justify-between">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <a href="{{ route('products.index') }}" class="bg-orange-500 text-white px-6 py-2 rounded-lg hover:bg-orange-600 transition-colors font-medium">
                        تسوق الآن
                    </a>
                </div>
            </div>
            ...
            <div class="flex overflow-x-auto scrollbar-hide">
                <button class="flex-shrink-0 px-4 sm:px-8 py-3 sm:py-4 text-orange-600 bg-orange-50 border-b-2 border-orange-600 font-semibold order-filter active whitespace-nowrap transition-colors text-sm sm:text-base" data-status="all">
                    <div class="flex items-center gap-1 sm:gap-2">
                        ...
                        <span class="bg-orange-100 text-orange-700 text-xs px-1.5 sm:px-2 py-1 rounded-full">{{ $orders->count() }}</span>
                    </div>
                </button>
                <button class="flex-shrink-0 px-4 sm:px-8 py-3 sm:py-4 text-gray-600 hover:text-yellow-600 hover:bg-yellow-50 font-semibold order-filter whitespace-nowrap transition-colors text-sm sm:text-base" data-status="pending">
                    <div class="flex items-center gap-1 sm:gap-2">
                        ...
                        <span class="bg-gray-100 text-gray-600 text-xs px-1.5 sm:px-2 py-1 rounded-full">{{ $orders->where('status', 'pending')->count() }}</span>
                    </div>
                </button>
                <button class="flex-shrink-0 px-4 sm:px-8 py-3 sm:py-4 text-gray-600 hover:text-orange-600 hover:bg-orange-50 font-semibold order-filter whitespace-nowrap transition-colors text-sm sm:text-base" data-status="confirmed">
                    <div class="flex items-center gap-1 sm:gap-2">
                        ...
                        <span class="bg-gray-100 text-gray-600 text-xs px-1.5 sm:px-2 py-1 rounded-full">{{ $orders->where('status', 'confirmed')->count() }}</span>
                    </div>
                </button>
                <button class="flex-shrink-0 px-4 sm:px-8 py-3 sm:py-4 text-gray-600 hover:text-purple-600 hover:bg-purple-50 font-semibold order-filter whitespace-nowrap transition-colors text-sm sm:text-base" data-status="shipped">
                    <div class="flex items-center gap-1 sm:gap-2">
                        ...
                        <span class="bg-gray-100 text-gray-600 text-xs px-1.5 sm:px-2 py-1 rounded-full">{{ $orders->where('status', 'shipped')->count() }}</span>
                    </div>
                </button>
                <button class="flex-shrink-0 px-4 sm:px-8 py-3 sm:py-4 text-gray-600 hover:text-green-600 hover:bg-green-50 font-semibold order-filter whitespace-nowrap transition-colors text-sm sm:text-base" data-status="delivered">
                    <div class="flex items-center gap-1 sm:gap-2">
                        ...
                        <span class="bg-gray-100 text-gray-600 text-xs px-1.5 sm:px-2 py-1 rounded-full">{{ $orders->where('status', 'delivered')->count() }}</span>
                    </div>
                </button>
                <button class="flex-shrink-0 px-4 sm:px-8 py-3 sm:py-4 text-gray-600 hover:text-red-600 hover:bg-red-50 font-semibold order-filter whitespace-nowrap transition-colors text-sm sm:text-base" data-status="cancelled">
                    <div class="flex items-center gap-1 sm:gap-2">
                        ...
                        <span class="bg-gray-100 text-gray-600 text-xs px-1.5 sm:px-2 py-1 rounded-full">{{ $orders->where('status', 'cancelled')->count() }}</span>
                    </div>
                </button>
            </div>
        </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 hover:shadow-md transition-all duration-200 order-item" data-status="{{ $order->status }}" data-order-id="{{ $order->order_number }}">
                ...
                <div class="p-4 sm:p-6 border-b border-gray-100">
                    ...
                    <div class="block lg:hidden">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="flex items-center gap-2 mb-1">
                                        <h3 class="text-base font-bold text-gray-900">#{{ $order->order_number }}</h3>
                                        <span class="{{ $order->status_badge_class }} px-2 py-1 rounded-full text-xs font-semibold">
                                            {{ $order->status_arabic }}
                                        </span>
                                    </div>
                                    <div class="text-xs text-gray-600 space-y-1">
                                        <div>{{ $order->created_at->format('j M Y') }}</div>
                            ...
                            <div class="relative">
                                ...
                                <div id="actions-{{ $order->order_number }}" class="hidden absolute left-0 top-full mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 z-10">
                                    <div class="py-2">
                                        ...
                                        <button onclick="trackOrder('{{ $order->order_number }}')" class="w-full text-right px-4 py-2 text-gray-700 hover:bg-gray-50 flex items-center gap-2 text-sm">
                                        ...
                                        <button onclick="cancelOrder('{{ $order->order_number }}')" class="w-full text-right px-4 py-2 text-red-700 hover:bg-red-50 flex items-center gap-2 text-sm">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                                            </svg>
                                            إلغاء الطلب
                                        ...
                                        <button onclick="reorderItems('{{ $order->order_number }}')" class="w-full text-right px-4 py-2 text-green-700 hover:bg-green-50 flex items-center gap-2 text-sm">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="flex items-center justify-between">
                            <div class="text-left">
                                <div class="text-xl font-bold text-gray-900">{{ number_format($order->total_amount, 2) }} د.أ</div>
                                @if($order->subtotal != $order->total_amount)
                                    <div class="text-xs text-gray-500">الأصلي: {{ number_format($order->subtotal + ($order->shipping_cost ?? 0), 2) }} د.أ</div>
                                @endif
                            </div>
                            
                            <!-- View Details Button - Mobile -->
                            <a href="{{ route('orders.show', $order->order_number) }}" class="bg-orange-500 text-white px-3 py-2 rounded-lg hover:bg-orange-600 transition-colors font-medium text-sm flex items-center gap-1">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="hidden lg:flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <a href="{{ route('orders.show', $order->order_number) }}" class="bg-orange-500 text-white px-4 py-2 rounded-lg hover:bg-orange-600 transition-colors font-medium text-sm flex items-center gap-2">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                    </svg>
                ...
                <div class="p-4 sm:p-6">
                    <div class="flex items-start justify-between mb-3 sm:mb-4">
                        <h4 class="font-semibold text-gray-900 text-sm sm:text-base">المنتجات ({{ $order->items->count() }})</h4>
                        @if($order->status == 'pending')
                    ...
                    <div class="space-y-3 sm:grid sm:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 sm:gap-4 sm:space-y-0">
                        ...
                        <div class="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                            ...
                            <div class="flex-1 min-w-0">
                                <h5 class="font-medium text-gray-900 truncate text-sm sm:text-base">{{ $item->product ? $item->product->name : 'منتج محذوف' }}</h5>
                                <div class="flex items-center justify-between mt-1">
                                    <span class="text-xs sm:text-sm text-gray-500">الكمية: {{ $item->quantity }}</span>
                                    <span class="font-semibold text-gray-900 text-sm sm:text-base">{{ number_format($item->total, 2) }} د.أ</span>
                                </div>
                            </div>
                        </div>
                        @endforeach
                        ...
                        <div class="flex items-center justify-center p-3 bg-gray-100 rounded-xl border-2 border-dashed border-gray-300 sm:col-span-1 lg:col-span-1">
                            <span class="text-gray-600 font-medium text-sm sm:text-base">+{{ $order->items->count() - 3 }} منتج آخر</span>
                        </div>
                        @endif
                    </div>
                </div>
