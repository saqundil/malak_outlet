...
<main class="container mx-auto px-3 sm:px-4" style="max-width: 1400px;">
    <div id="productsPageConfig" class="hidden" data-auth="{{ auth()->check() ? 1 : 0 }}"></div>
    <div class="min-h-screen bg-gray-50 px-4">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 lg:mb-8 space-y-4 sm:space-y-0 pt-4 sm:pt-6 lg:pt-8 px-2 sm:px-0">
            ...
            <div class="flex items-center space-x-4 space-x-reverse">
                <span class="text-sm sm:text-base text-gray-600">{{ $products->total() }} منتج</span>
            </div>
        </div>
                ...
                <div class="hidden lg:block lg:col-span-1">
                    <div class="bg-white rounded-xl shadow-lg border border-gray-100 sticky top-24 max-h-[calc(100vh-7rem)] overflow-y-auto filter-scrollbar">
                        ...
                        <div class="px-6 pb-6 filter-content">
                            ...
                            <!-- Sizes Filter -->
                            <div class="mb-6 {{ !request('category') ? 'hidden' : '' }}" id="sizes-filter-section">
                                            ...
                                            @foreach($availableSizes as $size => $count)
                                                <label class="relative cursor-pointer group">
                                                    ...
                                                    <div class="size-button w-full text-center py-3 px-2 text-xs font-medium border-2 rounded-xl transition-all duration-200 group-hover:border-orange-400 group-hover:shadow-md transform group-hover:scale-105
                                                          {{ in_array($size, request('sizes', [])) ? 'bg-gradient-to-r from-orange-500 to-orange-600 text-white border-orange-500 shadow-lg' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50' }}">
                    ...
                    <div class="bg-white rounded-lg sm:rounded-xl shadow-lg p-3 sm:p-4 mb-4 sm:mb-6 border border-gray-100 mx-2 sm:mx-0">
                        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-3 sm:space-y-0">
                            ...
                            <div class="flex items-center justify-between sm:justify-end space-x-3 space-x-reverse w-full sm:w-auto">
                                <span class="text-xs sm:text-sm text-gray-600">{{ $products->total() }} منتج</span>
                        ...
                        <div class="bg-white rounded-lg sm:rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 group border border-gray-100 flex flex-col h-full">
                            <div class="relative">
                                ...
                                <div class="absolute top-2 sm:top-3 right-2 sm:right-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                    <button class="bg-white/90 backdrop-blur-sm p-1.5 sm:p-2 rounded-full shadow-lg hover:bg-white hover:shadow-xl text-gray-600 add-to-wishlist-btn {{ in_array($product->slug, $wishlistProductIds ?? []) ? 'is-in-wishlist text-red-500' : '' }} transition-all duration-200"
                            ...
                            <div class="p-3 sm:p-4 flex flex-col flex-1">
                                ...
                                <!-- Product Category -->
                                @if($product->category)
                                    <p class="text-xs text-gray-500 mb-2 sm:mb-3">{{ $product->category->name }}</p>
                                @endif
                                ...
                                <div class="flex-1">
                                    ...
                                    <div class="mb-3 sm:mb-4">
                                        @if($product->has_discount)
                                            <div class="flex items-center gap-1 sm:gap-2 mb-1">
                                                <span class="text-base sm:text-lg font-bold text-orange-600">{{ number_format($product->final_price, 2) }} د.أ</span>
                                                <span class="text-xs sm:text-sm text-gray-500 line-through">{{ number_format($product->price, 2) }} د.أ</span>
                                            </div>
                                        @else
                                    ...
                                    @if(isset($product->average_rating) && $product->average_rating > 0)
                                        <div class="flex items-center mb-2 sm:mb-3">
                                            <div class="flex items-center">
                                                @for($i = 1; $i <= 5; $i++)
                                                    <svg class="w-3 h-3 sm:w-4 sm:h-4 {{ $i <= $product->average_rating ? 'text-yellow-400' : 'text-gray-300' }}" fill="currentColor" viewBox="0 0 20 20">
                                                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
                                                    </svg>
                                                @endfor
                                            </div>
                                            <span class="text-xs text-gray-600 mr-1 sm:mr-2">({{ $product->reviews_count ?? 0 }})</span>
                                        </div>
                                    @endif
                                </div>
                                    ...
                                    <div class="flex items-center gap-1.5 sm:gap-2">
                                        @if($product->quantity > 0 && $product->status == 'in_stock')
                                            ...
                                            @else
                                                ...
                                                <button onclick="addToCart(this, '{{ $product->slug }}')" 
            ...
            @else
                <div class="text-center py-12 lg:py-16 px-4">
                    ...
                    <a href="{{ route('products.index') }}" class="inline-flex items-center px-4 sm:px-6 py-2 sm:py-3 bg-gradient-to-r from-orange-500 to-orange-600 text-white font-medium rounded-lg hover:from-orange-600 hover:to-orange-700 transition-all duration-200 text-sm sm:text-base">
                        <svg class="w-4 h-4 sm:w-5 sm:h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
